appontoWeb.controller('ReportAdministratorController', function ($scope, $state, $http, $timeout, UserService, SystemService) {
    ...
    $scope.getReport = function () {
        ...
        $.each($scope.filter.selecteds, function (i, user) {
            userSearch.push(user.Id);
        })

        $http({
            url: SystemService.apiUrl + 'api/report/consolidated/between/?startDate=' + moment($scope.startDate).format('YYYY/MM/DD') + '&endDate=' + moment($scope.endDate).format('YYYY/MM/DD'),
            method: 'GET',
            ...
            $.each(data, function (i, result) {
                ...
                $.each(result.Results, function (i, day) {
                ...
                result.totalWorked = Math.floor(x.asHours()) + moment.utc(x.asMilliseconds()).format(":mm:ss")
            });
